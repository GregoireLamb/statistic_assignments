...
The generated sequence is defined based on the linear equation:
$x_{n+1} = (a  x_n + c) (modm)$
...
- We can obtain the quantile function of the cumulative density function $F(x) = 1 - exp(-\lambda x), \lambda > 0$ by inverting it. Which looks as follows $F_{x}^{-1}(u) = \frac{-ln(1-p)}{\lambda}$.
...
```{r}
plot(comp$test_c, comp$acc_rate, main = "Dependency of the acceptance rate on the constant c", xlab = "Constant c", ylab = "Acceptance rate")
```
