...
The best k can be achieved when mean(data) - c = 0, this can be achieved with c = mean(data) 
...
- For dataset x2 condition number is very high, which means that variance is less robust to the errors and it leads to higher error. There is a practical sense to perform shift corrections by sample mean to reduce the condition number k
