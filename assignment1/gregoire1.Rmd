...
Therefor we will use the *condition number*: $S = \sum_{i=1}^n (x_i - x_n)^2 = \frac{1}{(n-1)} * s_n^2$. It gives a idea of how a small change in the inputs will causes a change in the output. The closet is k to 1 the best it is because it would mean our variance remain trustful with some noise errors in the input.  
